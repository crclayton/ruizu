...
def get_album_dirs(source: Path):
    ...
    excluding the 'Random_Albums' folder itself.
    """
    albums = []
    skip = source / "Random_Albums"
...
def main():
    ...
    random.shuffle(album_dirs)

    # pair with creation time
    # dirs_with_ctime = []
    # for d in album_dirs:
    #     try:
    #         dirs_with_ctime.append((d, d.stat().st_ctime))
    ...
    for album in [d for d in album_dirs[:count]]:
