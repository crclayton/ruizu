...
#ffmpeg -i "$1" -y -vf "scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2,setsar=1" "scaled_$1"

#ffmpeg -i "scaled_$1" -y -filter_complex "\
#[0:v]tpad=stop_mode=clone:stop_duration=1,\
#pad=w=iw+160:h=ih+160:x=80:y=80:color=#000000[v]" \
#-map "[v]" -map 0:a? -c:v libx264 -crf 18 -preset veryfast -c:a copy "instagram_$1"
